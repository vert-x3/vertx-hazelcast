...
If you are using Vert.x from the command line, the jar corresponding to this cluster manager (it will be named `vertx-hazelcast-3.4.2-SNAPSHOT.jar`
...
the artifact: `io.vertx:vertx-hazelcast:3.4.2-SNAPSHOT` in your project.
...
You may want to use a different version of Hazelcast. The default version is `3.8`. To do so, you
need to:
...
----
dependencies {
 compile ("io.vertx:vertx-hazelcast:3.4.2-SNAPSHOT"){
