...
[CAUTION]
====
The beginning of this section is relevant only when using the default Hazelcast version, which is `${hazelcast.version}`.

Since version `5.0`, Hazelcast includes `hazelcast-kubernetes` and does not require an additional dependency.
For details about running Hazelcast 5 on Kubernetes, check out the https://docs.hazelcast.com/hazelcast/latest/kubernetes/kubernetes-auto-discovery[documentation].
====
...
The second step is to configure the discovery plugin inside your Hazelcast configuration, by either providing a custom `cluster.xml` file or programmatically, as described in <<configcluster>>.
